...
class Dashboards::Businesses::ApprovalsController < ApplicationController
  include DashboardLayout

  def create
    business = current_client.business

    return redirect_to dashboards_path if business.status != 'new' && business.status != 'rejected'
    ...
    return redirect_to(dashboards_path, alert: 'Need minimum of 100 characters') if word_count < 100
    ...
    if approval_histories.persisted?
      redirect_to dashboards_path, success: 'We will review your application.'
    else
      redirect_to dashboards_path, alert: approval_histories.errors.full_messages.join('. ')
    end
  end
